...
class GameRPS:
    ...
    def __check_player_rights(self, player):
        with get_connection() as conn:
            ...
            with conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                cur.execute('INSERT INTO honda_bot_users (id, name, battle_count_total, battle_count_win, battle_count_lose) VALUES (%s, %s, %s, %s, %s)',
                            (player_id, player_name, 0, 0, 0))
                return True, None, None
    ...
    def __update_player_access_and_battle_count(self, player, result):
        with get_connection() as conn:
            ...
            with conn.cursor(cursor_factory=DictCursor) as cur:
                ...
                cur.execute(
                    'UPDATE honda_bot_users SET last_accessed = %s WHERE id = %s', (dt_now, player_id))
                cur.execute(
                    'UPDATE honda_bot_users SET battle_count_total = battle_count_total + 1 WHERE id = %s', (player_id,))
                if result == 'W':
                    cur.execute(
                        'UPDATE honda_bot_users SET battle_count_win = battle_count_win + 1 WHERE id = %s', (player_id,))
                elif result == 'L':
                    cur.execute(
                        'UPDATE honda_bot_users SET battle_count_lose = battle_count_lose + 1 WHERE id = %s', (player_id,))
    ...
    # result: win->'W' lose->'L'
    def __create_rps_battle_string(self, player, hands, result='L'):
        player_hand = None
        honda_hand = None
        r, p, s = hands
        if result == 'W':
    ...
    async def __play_rps(self, ch, player, hands, m_prefix=""):
        ...
        m_prefix = m_prefix + \
            self.__create_rps_battle_string(player, hands, battle_result)
        ...
        if battle_result == 'W':
            ...
            if r is True:
                f = [discord.File(filename)
                     for filename in self.__filenames_lose_r]
                m = self.__msg_lose_r
            elif s is True:
                f = [discord.File(filename)
                     for filename in self.__filenames_lose_s]
                m = self.__msg_lose_s
            elif p is True:
                f = [discord.File(filename)
                     for filename in self.__filenames_lose_p]
                m = self.__msg_lose_p

        # update user's last access time when rps is done
        self.__update_player_access_and_battle_count(player, battle_result)
    ...
    async def process_message(self, client, message):
        ...
        # judge rights
        player_rights, dt_last_accessed, dt_prev_refresh = self.__check_player_rights(
            player)
        ...
        if hands.count(True) > 1:
            ...
            assert hands.count(True) == 1, 'assert: [r, p, s].count(True) == 1 ... r:{0}, p:{1}, s:{2}'.format(
...
def get_player_stats(player):
    with get_connection() as conn:
        ...
        with conn.cursor(cursor_factory=DictCursor) as cur:
            player_id = player.id
            cur.execute(
                'SELECT * FROM honda_bot_users WHERE id = %s', (player_id,))
            row = cur.fetchone()
            if row is None:
...
async def respond_allstats(message):
    if message.content.startswith("!allstats"):
        ...
        for player_id in player_ids:
            found, player_name, ttl, win, lose = get_player_stats_from_id_given(
                player_id)
            if found is True:
                win_rate = win/ttl
        ...
        if all_stats:
            ...
            for player_name, ttl, win, lose, win_rate in all_stats:
                string_list.append(
                    f"{player_name}さん：{win}勝{lose}敗 => 勝率 {(win/ttl):.2%}")
            m = "\n".join(string_list)
            await message.channel.send(m)
        else:
            m = textwrap.dedent(f"""\
